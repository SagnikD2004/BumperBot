                      ...
                      imu_angular_z_(0.0)
{
  ...
  imu_sub_ = create_subscription<sensor_msgs::msg::Imu>("imu/out", 100, std::bind(&KalmanFilter::imuCallback, this, _1));
...
void KalmanFilter::odomCallback(const nav_msgs::msg::Odometry &odom)
{
  ...
  if(is_first_odom_)
  {
    ...
    // RCLCPP_INFO(this->get_logger(), "Odometry Angular Z: %f", odom.twist.twist.angular.z);
...
void KalmanFilter::imuCallback(const sensor_msgs::msg::Imu &imu)
{
    ...
    // RCLCPP_INFO(this->get_logger(), "IMU Angular Z: %f", imu.angular_velocity.z);
}
...
void KalmanFilter::statePrediction()
{
    ...
    // RCLCPP_INFO(this->get_logger(), "State Prediction - Mean: %f, Motion: %f", mean_, motion_);
}
